...
Titles
Captions
Good aesthetics/accessible color palettes
legible font sizes
data point size (large or small enough)
...
Average sea temperature over time. Time, temperature, maybe location if comparing. Numeric + categoric. Convey change over time - line graph? Discreet points?
...
Line plot is probably the most effective for the above data set. 
...
Hue-based gradients are actually not ideal unless you are using it to indicate divergence from a normal/baseline. A better gradient to use is lightness-based, and an even better one uses two hues to form the gradient.
...
```{r}  
EnoDischarge <- mutate(EnoDischarge, Discharge.m3s = Discharge*.3048)
EnoDischarge <- mutate(EnoDischarge, Discharge.rollmean = rollmean(Discharge.m3s))
```
