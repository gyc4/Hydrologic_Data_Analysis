...
LAGOSnutrient2 <- LAGOSnutrient %>%
  select(sampledate, lagoslakeid, nh4, no2no3, chla, secchi, tn, tp) %>%
...
```{r}
LAGOSmatrix <- cor(LAGOSnutrient2[, c(3:8)])
corrplot(LAGOSmatrix, upper = "ellipse")

```

What patterns do you see?

> I see that tn correlates the closest with no2no3 (nitrates), while tp correlates the closest with chla (chlorophyl)

2. Create two linear regression models (hint: function `lm`) to predict TN and TP using your five predictor variables. 

```{r}
TNregression <- lm(data = LAGOSnutrient2, tn ~ no2no3 + nh4 + chla + secchi)
summary(TNregression)

TPregression <- lm(data = LAGOSnutrient2, tp ~ chla + no2no3 + nh4 + secchi)
...
> 56.27% of TN variance is being accounted for, and 43.85% of TP variance is being accounted for.
...
> It doesn't seem that I have to remove any of these variables based on their VIF values.
...
```{r}
TNrevisedregression <- lm(data = LAGOSnutrient2, tn ~ nh4 + chla + secchi)
summary(TNregression)

TPrevisedregression <- lm(data = LAGOSnutrient2, tp ~ no2no3 + nh4 + secchi)
summary(TPregression)
```
