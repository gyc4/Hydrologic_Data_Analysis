...
```{r}
ClearCreekSummary <- whatNWISdata(siteNumbers = "06719505") #summary of the kinds of data available from this site
```
...
ClearCreekDischarge <- readNWISdv(siteNumbers = "06719505",
                     parameterCd = "00060", # discharge (ft3/s) automatically gets the mean daily value
                     startDate = "",
                     endDate = "")
...
attr(ClearCreekDischarge, "variableInfo") #both metadata
...
ClearCreekRegressionPlot <- 
  ggplot(ClearCreekDischarge, aes(x = Date, y = Discharge)) +
  geom_line() +
  geom_smooth(method = "lm", se = FALSE, color = "#c13d75ff") + #Added a linear regression line to the gglineplot
  labs(x = "", y = expression("Discharge (ft"^3*"/s)")) + 
  theme(plot.title = element_text(margin = margin(b = -10), size = 12), 
       axis.title.x = element_blank())
...
> yes, if it has less/no seasonality. maybe if you're looking at discharge of one specific date over multiple years.
...
```{r}
ClearCreek_ts <- ts(ClearCreekDischarge[[4]], frequency = 365) #turn discharge data into a time series object. [4] specifies ONE column of data
#only being able to specify one column of data means our data must be EVENLY, TEMPORALLY SPACED.

#table(diff(ClearCreekDischarge$Date)) <- to check that no dates are missing
...
ggplot(ClearCreek_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
  geom_line(aes(y = seasonal, x = Date), color = "#c13d75ff") + #Add seasonal trend line
  geom_hline(yintercept = 0, lty = 2) +
  ylab(expression("Discharge (ft"^3*"/s)"))
