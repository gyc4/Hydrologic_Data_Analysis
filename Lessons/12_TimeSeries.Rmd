...
* **Change point detection**, e.g., `pettitt.test` (package: trend) or `breakpoints` (package: strucchange) <to use if you suspect there is a step-trend
...
> aggregation: to put data points together. could be mean, median, range. Basically to reduce the rows in the dataset useful when there's seasonality (take the average over a month to analyze trends)

> subsampling: to only consider one small portion of the data. useful when the sampling frequency varies and you're trying to standardize

> interpolation: to fill in gaps in the data (estimating intermediate values, etc)
...
* **Seasonal Mann-Kendall**: seasonality, non-parametric, no temporal autocorelation, identical distribution (needs the exact same frequency between each data point.) Function: `smk.test` (package: trend)
...
Mendotadata <- LAGOSnutrient %>%
  filter(lagoslakeid == 5371) %>% #gathers specific rows
  select(sampledate, tp) #gathers specific columns
...
Mendotadata <- Mendotadata %>%
  filter(tp < 300 & sampledate > "1988-01-01") %>% #data must fulfill both assumptions to be included in the new, filtered dataset 
  arrange(sampledate)
...
* **Piecewise constant**: <use the closest measured value for any missing values>
* **Linear**: <take the average between two points to get a missing value in the middle>
* **Spline**: <some kind of curved line?? might be able to catch missing highs/lows>
* **Seasonal Cycle**: <if you know there's a strong seasonal cycle, you can fill in a missing value with the averages of that one day in other years.
...
linearinterpolation <- as.data.frame(approx(Mendotadata, n = 303, method = "linear")) #approx gives us an interpolation. it doesn't automatically generate it as a data frame
...
> p-value > 0.05 means we can't reject the null (everything has stayed the same.) z score is the direction + magnitude of the trend. 
> October and November see significant downward trends. 
> In general, we cannot say for sure that there has been a significant change since the beginning/trend over time of total potassium levels. We can say, however, that TP levels in October and November in particular have seen statistically significant downward trends.
...
ClearCreekDischarge.Monthly <- ClearCreekDischarge %>%
  mutate(Year = year(Date), 
         Month = month(Date)) %>%
  group_by(Year, Month) %>% #group by both year AND month, not year and month separately 
...
#Therefore, we can infer that data are stationary. if the data weren't stationary, we would have to apply certain corrections
```
...
acf(ClearCreek_ts)
pacf(ClearCreek_ts)
#neither matches exactly to any expected functions. This is due to how multiple variables are involved
```
...
fit <- arima(ClearCreek_ts, c(1, 0, 1),seasonal = list(order = c(2, 1, 0), period = 12)) #fit our data with the arima model that fit the best

# make a prediction into the future
ClearCreekprediction <- predict(fit, n.ahead = 10*12) #can use this model to predict into the future
