...
install.packages("streamMetabolizer", dependencies=TRUE,
                 repos=c("https://owi.usps.gov/R","https://cran.rstudio.com"))
...
> hysteresis is caused by different kinds of water flow reaching the river at different times in the storm
...
> Diluting storm > one that increases flow but decreases concentration
> Flushing storm > one that increases flow AND increases concentation

> a diluting storm

```{r}
#creating the hysteresis loop
PotomacStorm <- Potdat %>%
...
> how fast an ecosystem consumes and produces energy/carbon

Why would we want to know the *GPP* and *ER* of a river reach?

> GPP > Gross Primary Productivity
> ER > Ecosystem Respiration - everything in the stream that respires carbon

> indicates a lot about the condition of the river
...
Eno.lon <- Enodat[[2]]$lon #Double brackets calls one iten in a list

Eno.DO <- Enodat[[1]] %>%
  spread(value = value, key = variable) %>%
  mutate(DateTime_Solar = convert_UTC_to_solartime(DateTime_UTC, Eno.lon)) #convert_UTC_to_solartime = function converts UTC to the solar time (what it would show on a sun dial at that exact location, at that exact time)
...
> P_t = Productivity at that time step (Photosynthesis)
> R_t = Respiration ""
> D_t = Difusion
