...
Dissolved O, nutrients (N, P, trace nutrients), microbes, color/odor, invasive species, contaiminants 
...
options(scipen = 100) #To not get e^x notation in ggplot values

lagosne_get(dest_folder = LAGOSNE:::lagos_path(), overwrite = TRUE) #only need to do once on a computer
...
LAGOSnutrient <- LAGOSdata$epi_nutr #nutrients in the epilimnion only
...
```{r}
# Join data frames
LAGOSlocations <- left_join(LAGOSlocus, LAGOSstate, by = "state_zoneid") #connect lagoslocus and lagosstate. left_join means we add ALL columns from the dataframe on the RIGHT to the dataframe on the LEFT.
...
Code that calculates trophic state index
...
```{r}
dim(LAGOSnutrient)
class(LAGOSnutrient$sampledate) #check that sampledate is being interpreted as a date in R

LAGOStrophic <- 
  left_join(LAGOSnutrient, LAGOSlocations, by = "lagoslakeid") %>% #Add columns in LAGOSlocations to LAGOSnutrient by lagoslakeid
  select(lagoslakeid, sampledate, chla, tp, secchi, 
         gnis_name, lake_area_ha, state, state_name) %>% #Select only relevant columns
  mutate(sampleyear = year(sampledate), 
         samplemonth = month(sampledate), 
         season = as.factor(quarter(sampledate, fiscal_start = 12))) %>% #break up sampledate to year, month, and season columns. Quarter divides a year into quarters. You can specify start month. as.factor treats the output month number as a factor.
  drop_na(chla:secchi) #if there is an N/A at all in a row, it will remove that entire row

levels(LAGOStrophic$season) <- c("Winter", "Spring", "Summer", "Fall") #name seasons
```

Let's look at observations of chl, secchi depth, and TP seasonally and over the period of study.
```{r}
chlbymonth <-  #chlorophyl
ggplot(LAGOStrophic, 
       aes(x = as.factor(samplemonth), y = chla, color = sampleyear)) +
  geom_jitter(alpha = 0.2) + #spreads out data points within the same y value along the x axis. good for high density plots. they plot on top of each other though
  labs(x = "Month", y = expression(Chlorophyll ~ a ~ (mu*g / L)), color = "Year") +
  scale_color_viridis_c(option = "magma")
print(chlbymonth)
#used to do more winter sampling than now? 
#most production in the summer
...
#concentrated more on the lower end

tpbymonth <- #phosphorus
ggplot(LAGOStrophic, 
...
print(tpbymonth)
#matches up more with chlorophyl graph
```
...
> more chlorophyl = less secchi depth
> more phosphorus = more chlorophyl
...
LAGOStrophic <- 
  mutate(LAGOStrophic, 
         TSI.chl = round(10*(6 - (2.04 - 0.68*log(chla)/log(2)))), #Computing trophic state index for all there variables
         TSI.secchi = round(10*(6 - (log(secchi)/log(2)))), #round() is rounding to the nearest integer
         TSI.tp = round(10*(6 - (log(48/tp)/log(2)))), 
         trophic.class = 
            ifelse(TSI.chl < 40, "Oligotrophic", #ifelse() assigns values in a new column, based on values in a different column. 
                   ifelse(TSI.chl < 50, "Mesotrophic", #else part for the above "if"
                          ifelse(TSI.chl < 70, "Eutrophic", "Hypereutrophic")))) #eutrophic is else part for above "if". Hypereutrophic is the else for the eutrophic's "if" (everything else will be labeled hypereutrophic)

#use chlorophyl because it's the most direct measurement of primary productivity out of the three
...
#scales::show_col(colormap(colormap = colormaps$magma, nshades=16))  <<< this is to grab specific colors from color palettes 
```
...
print(chlvssecchi)
#majority of points are below the 1to1 line
...
chlvstp <- ggplot(LAGOStrophic, aes(x = TSI.chl, y = TSI.tp)) +
  ...
  geom_abline(slope = 1, intercept = 0)  
print(chlvstp)
#similar relationship as above. majority of points are below the one to one line
```
...
#useful if u have different sample sizes etc..
...
stateTSIjitter <- ggplot(LAGOStrophic, aes(x = state, y = TSI.chl, color = season)) +
  geom_rect(xmin = -1, xmax = 19, ymin = 0, ymax = 40, #Rectangles represent trophic state thresholds 
...
> Violin: makes all categories seem the same (same width, etc). provides info on the distribution (range, medians)

> Bar: is clearer here the differing sample sizes

> Jitter: winter values are lower, summer values are higher. some states only take values during the summer
