...
class(waterfeatures) #it's already listed as a simple features data frame
...
ggplot(waterfeatures) +
  geom_sf(aes(fill = FEATURE, color = FEATURE)) + #call both fill and color to generate the nice boxes in the output legend
  scale_color_viridis_d(option = "magma", end = 0.9) + #end scale before the yellow end
  scale_fill_viridis_d(option = "magma", end = 0.9) #viridis_d = discrete!!!
...
NClayers <- ggplot() +
  geom_sf(data = HUC6.NC, aes(fill = Name), color = "darkgray", alpha = .6) +
  geom_sf(data = waterfeatures) +
  scale_fill_brewer(palette = "Paired") #qualitative palette
print(NClayers)
...
Neusebasin <- ggplot() +
  geom_sf(data = HUC6.Neuse, fill = "darkgray") + #code this first so everything else plots on top
...
> Streams, reservoirs (dams too), and shoreline. Intracoastal Waterway also runs through.
