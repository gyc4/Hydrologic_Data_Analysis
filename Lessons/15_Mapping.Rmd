...
LAGOScombined <- 
  left_join(LAGOSnutrient, LAGOSlocus) %>% #LAGOSlocus gives you latitude and longitude
...
Secchiplot <- ggplot() +
  ...
  geom_sf(data = secchi.spatial, aes(color = secchi.mean), #plot states first, then secchi depths on top
          alpha = 0.5, size = 1) +
...
```{r}
Maine.subset <- filter(states.subset, ID %in%
                         "maine")
Secchiplot <- ggplot() +
  geom_sf(data = states.subset, fill = "white") +
  geom_sf(data = secchi.spatial, aes(color = secchi.mean), #plot states first, then secchi depths on top
...
LAGOScombined <- LAGOScombined %>%
  mutate(decade = floor_date(sampledate, years (10)), #floor_date only gives us the first day of the decade year. it's a "rounding" function for dates
         decade = year(decade))
...
secchi.decade.spatial <- st_as_sf(secchi.summary.decade, coords = c("long", "lat"), crs = 4326) #turn it into a spatial dataframe. crs = (projection system)
...
Secchiplot <- ggplot() +
  ...
  geom_sf(data = secchi.decade.spatial, aes(color = secchi.mean), 
          alpha = 0.5, size = 1) +
  facet_wrap(vars(decade), ncol = 5) + #facet_wrap lets you specify decade column information
  scale_color_viridis_c() +
  labs(color = "Average Secchi Depth (m)") +
  theme(legend.position = "top")
print(Secchiplot)
...
```{r}
Secchi.Animation <- ggplot() + #p much same code as above, just without facet_wrap
  geom_sf(data = states.subset, fill = "white") +
  geom_sf(data = secchi.decade.spatial, aes(color = secchi.mean), 
          alpha = 0.5, size = 1) +
  scale_color_viridis_c() +
  ...
  transition_states(decade, state_length = 1, transition_length = 0) + #don't want transition bc otherwise the points move from one decade to the next
  enter_appear() +
  exit_disappear() 
