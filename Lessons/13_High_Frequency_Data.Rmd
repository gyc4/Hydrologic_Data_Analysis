...
> flash floods/fast storms, contaiminent spills, temperature over a day, oxygen content over a day
...
dischargedatTFC2018 <- readNWISuv( #<- new function, pulls high frequency data
...
> overland flow, goes into groundwater, directly in river, sewer pipes.
> storms are peaks in discharge in the graph
...
#we trying to separate base flow from quick flow
...
TFC2018 <- cbind(dischargedatTFC2018, TFCbaseflow) #binded together in the same order because they are the same length
...
#quick flow increases quickly, follows overall flow pretty closely in the early part of the storm
#quick flow is overland, direct contact flow
#base flow does not reach the stream as quickly. it peaks after quick flow peaks
#base flow involves percolation and a slow release

Export <- TFC2018 %>%
  mutate(timestep = c(diff(as.numeric(dateTime)), NA_real_), #calculating time intervals between measurements. NA_real_ --> 
         baseflowexport = bt * timestep, #multiplying rate by time elapsed to get total volume
         quickflowexport = qft * timestep) %>%
  summarize(BaseflowExport_cf = sum(baseflowexport, na.rm = T), #na.rm --> removes the NA value you added above
...
> Baseflow = 51.92%. Quickflow = 48.81%

Repeat this analysis for Ellerbe Creek. How do the percentages compare?

```{r}
ECbaseflow <- BaseflowSeparation(dischargedatEC2018$Flow_Inst, filter_parameter = 0.925, passes = 3)
...
> Increase, because urbanization creates a lot of impervious surfaces

What do you think "urban stream syndrom" refers to?

> Flashiness is a symptom of this. Urban streams are typically very flashy, deeper, and have water quality concerns
...
```{r Flashiness}
TFCsitedata <- readNWISsite(site = "02097280") #metadata
TFC.catchment.size <- TFCsitedata$drain_area_va #square miles

RBI_TFC2018 <- (
  sum(abs(diff(dischargedatTFC2018$Flow_Inst))) / #calculating the Richards-Baker Index!
    sum(dischargedatTFC2018$Flow_Inst[-1])
  ) / TFC.catchment.size

ECsitedata <- readNWISsite(site = "0208675010") #metadata
EC.catchment.size <- ECsitedata$drain_area_va #square miles

RBI_EC2018 <- (
  sum(abs(diff(dischargedatEC2018$Flow_Inst))) / #calculating the Richards-Baker Index!
...
> Ellebe Creek is flashier 

```{r}
whatNWISdata(parameterCD = "00060", statecd = "TX")
...
dischargedatCC <- readNWISuv(
  site = "07227448",
  parameterCd = "00060",
  stateDate = "2018-01-01",
  endDate = "2019-01-01"
) %>%
  renameNWISColumns()
```
...
> storm responses (total/base/quick flow) + biological changes/processes
